...
flags.DEFINE_string(
    "predict_file", "test_process4.jsonl",
...
def make_nq_answer(contexts, answer):
  ...
  print("XXXXXXXNQ ans ", contexts[start:end])
...
def read_nq_entry(entry, is_training):
  ...
  for i, question in enumerate(entry["questions"]):
    ...
    #print("XXXXX questions XXXXXXXX", questions)
    #print(doc_tokens)
    example = NqExample(
  ...
  if predictions:
    ...
    for c in example.candidates:
      ...
      if c["top_level"] and c["start_token"] <= start and c["end_token"] >= end:
        ...
        break
  #print("XXXXXXXXX short span", short_span)
  #print("XXXXXXXXX long span", long_span)
...
def load_and_run_model():
  ...
  print("Model loaded")
  return estimator,run_config, model_fn, tokenizer
# return (str, list)
def get_answer(estimator,run_config, model_fn, tokenizer,token_tuple_list, la_cndidates, url, doc_title, output,question):
  print("do pred XXXXXXXXXXX", FLAGS.do_predict)

    # If running eval on the TPU, you will need to specify the number of steps.
    ## copied to get answer
  if FLAGS.do_predict:
    if not FLAGS.output_prediction_file:
      raise ValueError(
          "--output_prediction_file must be defined in predict mode.")
    question, q_token_list = get_tokens_from_question(question)
    input_dict = get_input_json(token_tuple_list, la_cndidates, url, doc_title, output, question, q_token_list)
    eval_examples = read_nq_examples(
        input_dict, is_training=False)
    print("XXXX eval ex XXXX")
    print(eval_examples)
    ...
    if json["question"] == 'User connected':
        ...
        if readSource == 0:
            if json['source_type'] == 'link':
                ...
                estimator,run_config, model_fn, tokenizer = load_and_run_model()
        ...
        long_ans, shoert_anwers = get_answer(estimator,run_config, model_fn, tokenizer,token_tuple_list1, la_cndidates1, url1, doc_title1, output1,question_text)
        ans = "Long answer: " + long_ans
        ans += "Short answer : " + shoert_anwers[0]
        marked_ans = long_ans.replace(shoert_anwers[0],'<mark>'+shoert_anwers[0]+'</mark>')
...
if __name__ == "__main__":
  ...
  #load_and_run_model()
  print('model loaded')
  socketio.run(app,debug=False, host='172.16.11.23')
  tf.app.run()
