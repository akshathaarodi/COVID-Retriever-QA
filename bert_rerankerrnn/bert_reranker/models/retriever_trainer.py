    ...
    def step_helper(self, batch):
        ...
        if self.loss_type == 'negative_sampling':
            ...
            if self.retriever.returns_embeddings:
                ...
                logits = self.retriever(**inputs)
            #print("logits and targets are", logits, targets)
            #print("Type of target is", targets.type(), logits.type())
            loss = self.cross_entropy(logits, targets)
            #l = nn.BCEWithLogitsLoss()
            #logit_f = logits.type(torch.FloatTensor).to('cuda')
            #targets_f = targets.type(torch.FloatTensor).to('cuda')
            #logits = logit_f
            #loss = l(logit_f,targets_f)
